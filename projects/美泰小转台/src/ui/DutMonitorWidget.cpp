...
DutMonitorWidget::DutMonitorWidget(QWidget *parent)
    ...
    , m_columnsPerRow(3)  // 3 columns per row by default
{
    setupUi();
    m_dutManager = nullptr;
}
...
void DutMonitorWidget::setDutManager(Services::DutManager* dutManager)
{
    if (m_dutManager) {
        disconnect(m_dutManager, nullptr, this, nullptr);
    }

    m_dutManager = dutManager;

    if (m_dutManager) {
        connect(m_dutManager, &Services::DutManager::siteConfigurationLoaded, 
                this, &DutMonitorWidget::onSiteConfigurationLoaded);
        connect(m_dutManager, &Services::DutManager::siteChipPlacementChanged,
                this, &DutMonitorWidget::onSiteChipPlacementChanged);
        connect(m_dutManager, &Services::DutManager::siteUpdated,
                this, &DutMonitorWidget::onSiteUpdated);
        connect(m_dutManager, &Services::DutManager::dutStateChanged,
...
{
    LOG_MODULE_INFO("DutMonitorWidget", QString("DutMonitorWidget::onSiteUpdated Updating site '%1' ip '%2' from scan data...").arg(siteAlias).arg(ip).toStdString());
