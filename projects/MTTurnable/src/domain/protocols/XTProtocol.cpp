...
{
    uint32_t gyro_x = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_y = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_z = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_acc_x = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_acc_y = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_acc_z = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_mix = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint16_t gyro_temperature = Utils::ByteUtils::readLittleEndian<uint16_t>(data);
    
    // 将0xFF值转换为-1进行显示
    dut.gyro_x = (gyro_x == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_x;
    dut.gyro_y = (gyro_y == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_y;
    dut.gyro_z = (gyro_z == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_z;
    dut.gyro_acc_x = (gyro_acc_x == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_acc_x;
    dut.gyro_acc_y = (gyro_acc_y == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_acc_y;
    dut.gyro_acc_z = (gyro_acc_z == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_acc_z;
    dut.gyro_mix = (gyro_mix == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_mix;
    dut.gyro_temperature = (gyro_temperature == 0xFFFF) ? static_cast<uint16_t>(-1) : gyro_temperature;
}
...
void XTProtocol::parseExternalGyroData(const uint8_t* data, ExternalGyroData& gyro)
{
    ...
    uint32_t gyro_acc_z = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint32_t gyro_mix = Utils::ByteUtils::readLittleEndian<uint32_t>(data); data += 4;
    uint16_t gyro_temperature = Utils::ByteUtils::readLittleEndian<uint16_t>(data); data += 2;
    uint16_t gyro_counter = Utils::ByteUtils::readLittleEndian<uint16_t>(data);
    
    // 将0xFF值转换为-1进行显示
    gyro.gyro_x = (gyro_x == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_x;
    gyro.gyro_y = (gyro_y == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_y;
    gyro.gyro_z = (gyro_z == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_z;
    gyro.gyro_acc_x = (gyro_acc_x == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_acc_x;
    gyro.gyro_acc_y = (gyro_acc_y == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_acc_y;
    gyro.gyro_acc_z = (gyro_acc_z == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_acc_z;
    gyro.gyro_mix = (gyro_mix == 0xFFFFFFFF) ? static_cast<uint32_t>(-1) : gyro_mix;
    gyro.gyro_temperature = (gyro_temperature == 0xFFFF) ? static_cast<uint16_t>(-1) : gyro_temperature;
    gyro.gyro_counter = (gyro_counter == 0xFFFF) ? static_cast<uint16_t>(-1) : gyro_counter;
}
            ...
            if (auto data = parseTestFeedbackPacket(cmdData)) {
                ...
                for (int i = 0; i < 8; ++i) {
                    if ((data->dutActive >> i) & 0x01) {
                        ...
                        stream << "    Temperature: " << data->dutData[i].gyro_temperature / 100.0 << " C\n";
                    }
                }
                ...
                if ((data->dutActive >> 15) & 0x01) {
                    ...
                    stream << "    Temperature: " << data->externalGyro.gyro_temperature / 100.0 << " C\n";
