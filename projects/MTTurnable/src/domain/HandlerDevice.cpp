...
bool HandlerDevice::initialize()
{
    ...
    LOG_MODULE_INFO("HandlerDevice", QString("Initializing SProtocol with config: %1")
...
QJsonObject HandlerDevice::executeCommand(const QString &command, const QJsonObject &params)
{
    ...
    try {
        if (command == "sendCmd3Command"||command == "loadTask") {
            ...
            // 如果taskPath包含变量引用，从配置中解析
            if (taskPath.contains("${device.handler-01.cmd3TaskPath}")) {
                taskPath = m_configuration.value("cmd3TaskPath").toString();
                if (taskPath.isEmpty()) {
                    taskPath = "C:\\TestTask\\demo.tsk"; // 默认路径
                }
            }
            ...
            QByteArray resultData = QByteArray::fromHex(params["result"].toString().toLatin1());
