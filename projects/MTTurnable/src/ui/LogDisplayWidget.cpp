    ...
    , m_totalLogCount(0)
{
    ...
    // 注册spdlog sink
    m_sinkCallbackId = QString("LogDisplayWidget_%1").arg(reinterpret_cast<quintptr>(this));
    TesterFramework::Logger::getInstance().addSink([this](TesterFramework::LogLevel level, const std::string& message) {
        // 在GUI线程中处理日志
...
{
    // 移除spdlog sink（这里Logger没有提供移除sink的方法，所以暂时留空）
}
