...
//Log.PrintLog(0, "HopNum:%d ,DevName:%s", DeviceModel.HopNum(), DeviceModel.DevName().toStdString().c_str());
//
//DeviceModel.setHopNum(123); 直接设置
//Log.PrintLog(0, "HopNum:%d ", DeviceModel.property("HopNum").toInt());
//
//pObj->setProperty("HopNum", 1000); //通过属性访问
//Log.PrintLog(0, "HopNum1:%d ", DeviceModel.HopNum());
...
public:
	...
	/// 设置设备信息
	/// </summary>
	/// <param name="paraDev">设备信息数据结构</param>
	void SetDevStuInfo(DeviceStu paraDev);

	/// <summary>
	/// 获取设备信息
	/// </summary>
	/// <returns>返回设备数据信息</returns>
	DeviceStu GetDevStuInfo();
...
typedef QMap<std::string, tDeviceModelHash> tDeviceModelChainMap;
typedef QMap<std::string, tDeviceModelHash>::iterator tDeviceModelChainMapItr;
#endif 