...
private:
	...
	bool InsertImpl(bool is_insert, T&& t) {
		...
		StmtRAII stmt;
        int ret = sqlite3_prepare_v2(db_, sql.c_str(), sql.size(), &stmt.GetStmt(), nullptr);
        qDebug() << "sqlite3_prepare_v2 return :" << ret;
        if (ret != SQLITE_OK) {
			last_err_ = sqlite3_errmsg(db_);
            qDebug() << QString::fromStdString(last_err_);
			return false;
		}
    ...
    void SetMemVar(QString& mem_var, sqlite3_stmt* stmt, int i) const;
    ...
    void BindMemVar(unsigned long& mem_var, sqlite3_stmt* stmt, int i) const;
    void BindMemVar(bool& mem_var, sqlite3_stmt* stmt, int i) const;
    void BindMemVar(QString& mem_var, sqlite3_stmt* stmt, int i) const;
