/*
 aes_modes.c - These functions implement multiple block AES modes for ECB, CBC, CFB,
               OFB and CTR encryption,  The code provides support for the VIA Advanced
               Cryptography Engine (ACE).

  NOTE:        If VIA ACE is being used, these functions require the AES contexts (ctx) 
...
AES_RETURN unwrapfile256(const unsigned char* key, const unsigned char* iv, const unsigned char* ibuf, unsigned char* obuf) {
    aes_decrypt_ctx ctx[1];
	unsigned char* A, B[16], *R;
	unsigned int i, j, t, len = 32;

	if (aes_decrypt_key256(key, ctx) != EXIT_SUCCESS)
        return -1;

    A = B;
    t = 6 * (len >> 3);
    memcpy(A, ibuf, 8);
    memcpy(obuf, ibuf + 8, len);
    for (j = 0; j < 6; j++) {
        R = obuf + len - 8;
        for (i = 0; i < len; i += 8, t--, R -= 8) {
...
static int o_offset = 0;
static void to_top() {
    i_offset = 0;
    o_offset = 0;
}

static unsigned long read_enc(unsigned char* obuf, const unsigned char* ibuf, int ilen, int size) {
	int last_num = 0;
	if (size == 0 || i_offset == ilen)
		return 0;

	if (i_offset + size >= ilen) {
		last_num = ilen - i_offset;
		memcpy(obuf, ibuf + i_offset, last_num);
		i_offset = ilen;
		return last_num;
	}

    memcpy(obuf, ibuf + i_offset, size);
    i_offset += size;
    return size;
}

static unsigned long write_dec(const unsigned char* ibuf, unsigned char* obuf, int olen, int size) {
    if (size == 0 || o_offset + size > olen)
        return 0;

    memcpy(obuf + o_offset, ibuf, size);
    o_offset += size;
    return size;
}

AES_RETURN decodefile256(const unsigned char* key, const unsigned char* ibuf, int ilen, unsigned char* obuf, int olen) {
	aes_decrypt_ctx ctx[1];
	unsigned char dbuf[3 * BLOCK_LEN], buf[BLOCK_LEN];
	unsigned long i, len, wlen = BLOCK_LEN, tlen = 0;
    ...
    // When ciphertext stealing is used, we use three ciphertext blocks so
	...
	to_top();
	len = read_enc(dbuf, ibuf, ilen, 2 * BLOCK_LEN);
    ...
    if (len < 2 * BLOCK_LEN) {	// the original file is less than one block in length
        ...
        if (write_dec(dbuf, obuf, olen, len) != len)
            return 0;
        tlen += len;
        ...
        while (1) {	// while some ciphertext remains, prepare to decrypt block b2
            ...
            len = read_enc(b3, ibuf, ilen, BLOCK_LEN);
            ...
            if (len == 0 || len == BLOCK_LEN) {    // no ciphertext stealing
                ...
                if (write_dec(b3, obuf, olen, BLOCK_LEN) != BLOCK_LEN)
                    return 0;
                tlen += BLOCK_LEN;
            }

            // write the decrypted plaintext block
            if (write_dec(buf, obuf, olen, wlen) != wlen)
                return 0;
            tlen += wlen;
