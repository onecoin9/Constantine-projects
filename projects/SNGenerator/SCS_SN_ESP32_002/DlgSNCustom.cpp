...
#include "stdafx.h"
#include "SCS_SN_ESP32_002.h"
#include "DlgSNCustom.h"
#include "../Com/ComFunc.h"

// CDlgSNCustom 对话框

#define DEFAULT_ADDR (0x3FF000)
...
BOOL CDlgSNCustom::SaveCtrls()
{
	...
	CString strIniFile;
	CString strTemp;
	strIniFile.Format("%s\\sngen\\SCS_SN_ESP32_002.ini", CComFunc::GetCurrentPath());
...
INT CDlgSNCustom::QuerySN(UINT AdapIdx,DWORD Idx,BYTE*pData,INT*pSize)
{
	...
	//以下是pData的Header内容
	lSerial<<GroupCnt;//包含几处SN
	lSerial<<pCfg->DeviceIDAddr;//界面得到
	nTotalLen += (4); //CRC(4)
	lSerial<<nTotalLen;

	//以下是SNData的内容
	char *buffer = (char *)malloc(nTotalLen);
	int offset = 0;
	memcpy(buffer + offset, (BYTE*)ParamCol.strProdKeyCol.GetBuffer(), ParamCol.strProdKeyCol.GetLength());
	ParamCol.strProdKeyCol.ReleaseBuffer();
	offset += ParamCol.strProdKeyCol.GetLength();

	memcpy(buffer + offset, (BYTE*)ParamCol.strDevNameCol.GetBuffer(), ParamCol.strDevNameCol.GetLength());
	ParamCol.strDevNameCol.ReleaseBuffer();
	offset += ParamCol.strDevNameCol.GetLength();

	memcpy(buffer + offset, (BYTE*)ParamCol.strDevSecretCol.GetBuffer(), ParamCol.strDevSecretCol.GetLength());
	ParamCol.strDevSecretCol.ReleaseBuffer();
	offset += ParamCol.strDevSecretCol.GetLength();

	lSerial.SerialInBuff((BYTE*)buffer, offset);
		
	unsigned int crc32result = htonl(get_mem_crc32(buffer, offset));
	lSerial<<crc32result;
