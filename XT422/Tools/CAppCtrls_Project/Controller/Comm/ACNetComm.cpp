...
int CNetComm::SetSocketOpts(tSocketOpts* pSocketOpts)
{
	...
	if (pSocketOpts) {
		...
		pDestSocketOpts->RecvBufSize = 64 * 1024 * 1024;
		pDestSocketOpts->SendBufSize = 64 * 1024 * 1024;
...
int32_t CNetComm::SendData(uint8_t* pData, int32_t Size)
{
	int32_t Ret = 0;
	int32_t BytesSend = 0;
	...
	_PrintLog(LOGLEVEL_D, "CNetComm::SendData BytesSend = %d\r\n", BytesSend);
	if (BytesSend <= 0) {
...
{
	_PrintLog(0, "ThreadExit RetCode:%d\r\n",RetCode);
...
int32_t CNetComm::SetupNetWork()
{
	...
	if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0){
		_PrintLog(LOGLEVEL_E,"WSAStartup failed");
	...
	if (m_LocalSocket == SOCKET_ERROR){
		_PrintLog(LOGLEVEL_E, "create socket failed");
	...
	_PrintLog(LOGLEVEL_N, "NetComm Local[%s:%d], Remote[%s:%d]\r\n", m_Local.Addr.toStdString().data(), m_Local .Port,\
				m_Remote.Addr.toStdString().data(),m_Remote.Port);
	SktErr= bind(m_LocalSocket, (struct sockaddr*)&m_LocalAddr, sizeof(struct sockaddr));
	if (SktErr != 0) {
		_PrintLog(LOGLEVEL_E, "Bind Socket Error\r\n");
	...
	if (SktErr != 0) { 
		_PrintLog(LOGLEVEL_E, "SetSockOpt SO_RCVBUF Error\r\n");
	...
	if (SktErr != 0) {
		_PrintLog(LOGLEVEL_E, "SetSockOpt SO_SNDBUF Error\r\n");
		Ret = ERR_NETCOMM_SetSktOpts;
		goto __end;
	}
	...
	if (SktErr != 0) {
		_PrintLog(LOGLEVEL_E, "SetSockOpt SO_REUSEADDR Error\r\n");
		Ret = ERR_NETCOMM_SetSktOpts;
		goto __end;
	}
	...
	if (SktErr != 0) {
		_PrintLog(LOGLEVEL_E, "SetSockOpt SO_SNDTIMEO Error\r\n");
		Ret = ERR_NETCOMM_SetSktOpts;
		goto __end;
	}
	...
	if (SktErr != 0) {
		_PrintLog(LOGLEVEL_E, "SetSockOpt SO_RCVTIMEO Error\r\n");
		Ret = ERR_NETCOMM_SetSktOpts;
		goto __end;
	}
...
__end:
	if (SktErr) {
		_PrintLog(LOGLEVEL_E, "SocketErrorCode:%d\r\n", WSAGetLastError());
	}
	return Ret;
}
...
int32_t CNetComm::HandleMsg(uint8_t* PckData, int32_t PckSize)
{
	int32_t Ret = 0;
	if (m_pCmdHandler) {
		Ret=m_pCmdHandler->HandlePacket(PckData, PckSize);
	}
	else {
		_PrintLog(LOGLEVEL_E, "CmdHandler=NULL\r\n");
	}
	return Ret;
}
...
void CNetComm::ThreadHandler()
{
	...
	int32_t CurIdx=0;
	int32_t TimeRuns = 2;
	int32_t nSize = sizeof(SOCKADDR);
	_PrintLog(LOGLEVEL_N, "Run In Thread...\r\n");
	...
	while (!m_bStopThead){
		...
		int32_t BytesRead = recvfrom(m_LocalSocket, (char*)TmpDataBuf, MaxNetDataSize, 0, &clntAddr,&nSize);
		//_PrintLog(LOGLEVEL_N, "Recv Bytes:%d\r\n", BytesRead);
		if (BytesRead>=0){
			Ret=HandleMsg(TmpDataBuf, BytesRead);
		}
		else{
			//错误码对应信息 https://learn.microsoft.com/zh-cn/windows/win32/winsock/windows-sockets-error-codes-2
			//_PrintLog(LOGLEVEL_E, "Recv Data Error:%d\r\n", WSAGetLastError());
		}
	}
