...
namespace Utils
{
	...
	void AngKCommonXMLParser::DriverFeature_Json(pugi::xml_node& rootNode, unsigned long ulAlgo, nlohmann::json& outJson)
	{
		pugi::xml_node driverFeatureTable = rootNode.child(XML_NODE_CHIPDATA_DRIVERFEATURETABLE);
		int driverFeatureCount = std::distance(driverFeatureTable.begin(), driverFeatureTable.end());
		for (int i = 0; i < driverFeatureCount; ++i)
		{
			pugi::xml_node uidNode = driverFeatureTable.child(XML_NODE_CHIPDATA_UIDSUPPORT);
			outJson[XML_NODE_CHIPDATA_UIDSUPPORT] = uidNode.text().as_string();
			pugi::xml_node securityNode = driverFeatureTable.child(XML_NODE_CHIPDATA_SECURITYSOLUTION);
			outJson[XML_NODE_CHIPDATA_SECURITYSOLUTION] = securityNode.text().as_string();
			pugi::xml_node testNode = driverFeatureTable.child(XML_NODE_CHIPDATA_TESTSOLUTION);
			outJson[XML_NODE_CHIPDATA_TESTSOLUTION] = testNode.text().as_string();
			break;
		}
		return;
	}
