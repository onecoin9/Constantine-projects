...
qint64 CUplinkDataReceiver::WriteDataByDataWriter(char* pData, qint64 Size)
{
    ...
    if (m_pIDataWriter) {
        ...
        ALOG_FATAL("WriteDataByDataWriter m_pIDataWriter=NULL.", "CU", "--");
    }
    m_DataWriterMutex.unlock();
    return WriteBytes;
}

void CUplinkDataReceiver::onSLotDoWork()
{
    //ALOG_INFO("Begin Work");
}
...
void CUplinkDataReceiver::OnRecvDataUplink(const QByteArray PckData)
{
    ...
    //_PrintLog(LOGLEVEL_D, "CUplinkDataReceiver::DataArray Ptr:0x%I64X, Data[0]=0x%08X, PackNum:%d", PckData.constData(), *(uint32_t*)pDataPacket->Data,pDataPacket->PNum);
	if (pDataPacket->FristFlag == 1) {
		m_PacketCntIdx = 0;
		if (pDataPacket->PNum != 0) {
			ALOG_FATAL("FirstFlag=1,But PNum=%d.", "CU", "--", pDataPacket->PNum);
		}
	}

	if (pDataPacket->PNum != m_PacketCntIdx) {
		ALOG_DEBUG("CUplinkDataReceiver DataLost::DataArray PackNumRecv:%d,Expected:%d,CurTotalPacket:%d,Length:0x%08X", "CU", "--", pDataPacket->PNum, m_PacketCntIdx, m_PacketTotal, pDataPacket->Length * 4);
