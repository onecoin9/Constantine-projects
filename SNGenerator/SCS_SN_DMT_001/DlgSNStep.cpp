		...
		if(subCmd==CMD_GETTEXTLIMIT){
			if(nColumn==TAG_SNSTARTVALUE){
				if(pSnGroup->dwSNMode==SNMODE_ASCII){
					...
					UINT uMax = pSnGroup->dwSNLen*2;
					if (pSnGroup->dwSNStyle == SNSTYLE_BCD){
						uMax = pSnGroup->dwSNLen*5;
					}
					return uMax;
				}
			}
	...
	switch(nColumn){
		...
		case TAG_SNSTARTVALUE:
			if (pSnGroup->dwSNStyle==SNSTYLE_BCD){
				...
				double dbValue = 0;
				CString strConv;
				strConv.Format("%s",strText );
				if (StrDec2StrHex(strConv) == FALSE){
					CString strErrMsg;
					strErrMsg.Format("SN Error: %s contain invalid character",strText);
					MessageBox(strErrMsg);
					goto __end;
				}
				pTemp = new BYTE[pSnGroup->dwSNLen];
				if (pTemp == NULL){
					goto __end;
				}
				memset(pTemp, 0xFF, pSnGroup->dwSNLen);
				int nCurSize = strConv.GetLength();
				int nSetSize =  pSnGroup->dwSNLen*2;
				if (nCurSize > nSetSize){
					pSnGroup->strSNStartValue.Format("%d", 0);
					m_ListCtrlST.SetItemText(nRow,TAG_SNSTARTVALUE,pSnGroup->strSNStartValue);
					AfxMessageBox("The preset text limit was exceeded");
					goto __end;
				}
			}
...
INT CDlgSNStep::QuerySN(DWORD Idx,BYTE*pData,INT*pSize)
{
	...
	for(i=0;i<GroupCnt;++i){
		...
		for (int k = 0; k < SNSize; k++){
			pWriteBuffer[2*k/*+1*/] = pSNData[k];
		}
		
		lSerial.SerialInBuff(pWriteBuffer,SNSize*2);
	}
