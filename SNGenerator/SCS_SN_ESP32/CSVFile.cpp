...
INT CCSVFile::ReadSN(UINT Index, tParamCol &ParamCol, INT &nTotalLen)
{
	...
	ParamCol.strDevNameCol = strArrayCol[0];
	nTotalLen += strArrayCol[0].GetLength();

	ParamCol.strDevSecretCol = strArrayCol[1];
	nTotalLen += strArrayCol[1].GetLength();

	ParamCol.strProdKeyCol = strArrayCol[2];
	nTotalLen += strArrayCol[2].GetLength();

	ParamCol.strProdSecretCol = strArrayCol[3];
	nTotalLen += strArrayCol[3].GetLength();

	ParamCol.strPidCol = strArrayCol[4];
	nTotalLen += strArrayCol[4].GetLength();

	strMacTmp = strArrayCol[5];
	strMacTmp.Replace(":", "");
	if (strMacTmp.GetLength() != MAC_LEN_BYTE_SIZE*2){
		m_strErrMsg.Format("Mac convert error, Content:%s",strOneLine);
		Ret = -1; goto __end;
	}

	convertLen = Str2HexArray((UCHAR *)strMacTmp.GetBuffer(), ParamCol.MacCol, MAC_LEN_BYTE_SIZE);
	if (convertLen != MAC_LEN_BYTE_SIZE){
		m_strErrMsg.Format("Mac convert error, strMac:%s convertLen:%d",strMacTmp, convertLen);
		Ret = -1; goto __end;
	}
	nTotalLen += MAC_LEN_BYTE_SIZE;
	if(Ret!=0){
		goto __end;
	}
...
INT CCSVFile::OpenFile(CString strFilePath)
{
	...
	CString strOneLine;
	CStringArray strArrayCol;
	CString strDevNameCol,strDevSecretCol, strProdKeyCol,strProdSecretCol, strPidCol, strMacCol;
	m_IsReady=FALSE;
	...
	strDevNameCol = strArrayCol[0];
	strDevSecretCol = strArrayCol[1];
	strProdKeyCol = strArrayCol[2];
	strProdSecretCol = strArrayCol[3];
	strPidCol = strArrayCol[4];
	strMacCol = strArrayCol[5];

	if(strDevSecretCol.CompareNoCase("DeviceSecret")==0 && strProdKeyCol.CompareNoCase("ProductKey")==0 &&
		strProdSecretCol.CompareNoCase("ProductSecret")==0 && strPidCol.CompareNoCase("Pid")==0 &&
		strMacCol.CompareNoCase("Mac")==0 && strDevNameCol.CompareNoCase("DeviceName")==0){
		///Fix the
