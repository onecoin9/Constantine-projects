...
BOOL CDlgSNCustom::SaveCtrls()
{
	...
	if(sscanf(strText,"%I64X",&pCfg->DeviceIDAddr)!=1){
		MessageBox("»ñÈ¡DeviceIDÆðÊ¼µØÖ·´íÎó,ÇëÈ·ÈÏ");
		return FALSE;
	}*/
...
INT CDlgSNCustom::QuerySN(UINT AdapIdx,DWORD Idx,BYTE*pData,INT*pSize)
{
	...
	CString strUtcTime;
	CString strContent;
	CString str_product_model;
	CHMAC_SHA1 HMAC_SHA1;
	BYTE pOut[4096]={0};
	char product_model[4096]={0};
	cJSON* item = NULL;
	...
	strUtcTime.Format("%s GMT", szTemp);
	
	strContent.Format("\
X-Date:%s\n\
Source:%s\
",strUtcTime, m_tCfg.strCode);
	
	//HMAC_SHA1.HMAC_SHA1( (BYTE*)(LPCSTR)strContent, strContent.GetLength(),  (BYTE*)(LPCSTR)m_tCfg.strCode, m_tCfg.strCode.GetLength(), pOut) ;
	////ComTool::Hex2Str(ComTool::ENDIAN_LIT,pOut, strlen(pOut), strTemp);
	////base64����
	//int RealSize = 0;
	...
	//}
	//BYTE EnCode[4096] ={0};
	//int SizeConvert =base64_encode((uint8*)pOut, RealSize, EnCode);
	//if (SizeConvert <= 0){
	//	Ret = -1; goto __end; 
	//}
	//ComTool::Hex2Str(ComTool::ENDIAN_LIT,EnCode, SizeConvert, strTemp);
	
	strAuthorization.Format("hmac id=\"%s\",algorithm=\"%s\",headers=\"x-date source\",signature=\"%s\"", 
		m_tCfg.strCode, m_tCfg.strAlgorithm, strTemp);
...
Source:%s\r\n\
Authorization:%s\r\n\
", strUtcTime, m_tCfg.strCode, strAuthorization);

	cJSON* RootObj=cJSON_CreateObject();
	cJSON_AddStringToObject(RootObj, "outsourcing",(LPSTR)(LPCSTR)m_tCfg.strOrderID); 
	strPostData = cJSON_Print(RootObj);
//�����ȡģ��ֵ
	Ret = Client.HttpPost(m_tCfg.strDomain, strHeader, strPostData, strResponse);
	if (Ret != 0){
		Ret=-1; goto __end;
	}

	m_FileLog.PrintLog(LOGLEVEL_ERR, "http���ʻ�ȡ����ԭʼ����%s", strResponse);

	
	cJSON* Root = cJSON_Parse((LPSTR)(LPCSTR)strResponse);
	if (Root == NULL){
		Ret=-1; goto  __end;		
	}
	...
	item=cJSON_GetObjectItem(obj, "product_model"); 	
	str_product_model.Format("%s", item->valuestring);

	memcpy(product_model, str_product_model, str_product_model.GetLength());

	if (m_tCfg.nMode == 0){
		//��ȡ��Ԫ�飻
		m_DllHelp.GetEncrypTuple(product_model, path, reg, msg, TupleOut, len, len);
	}else{ //��ȡ��Ԫ��
		m_DllHelp.GetPlatformGetEncrypTuple_BLE(product_model, path, reg, msg, TupleOut, len, len);
		//m_DllHelp.GetTuple_BLE(path, reg, TupleOut, len);
	}

	if (*reg != 1){
		AfxMessageBox("������ERP��������ȡԪ������ʧ��");
		Ret=-1; goto __end; 
	}
	...
	if (m_tCfg.nMode == 0){ //��Ԫ��
		for(int i = 0; i < 8; i++){
			memcpy(MacArr+ 14 - (i*2), TupleOut+i*2, 2);
		}
		memcpy(TupleOut, MacArr, 16); //Mac�����ֽ�swap
	}else{//��Ԫ��

		for(int i = 0; i < 6; i++){
			memcpy(MacArr+ 10 - (i*2), TupleOut+i*2, 2);
		}
		memcpy(TupleOut, MacArr, 12); //Mac�����ֽ�swap
	}

	
	
	strSN.Format("%s", TupleOut); //»ñÈ¡µ½µÄSNºÅ
	vHex.clear();

	m_FileLog.PrintLog(LOGLEVEL_ERR, "Swap�������%s", strSN);

	if (m_tCfg.nMode == 0 || true){ //��Ԫ��
		ComTool::Str2Hex(strSN, ComTool::ENDIAN_BIG, vHex);
		for (int n = 0; n < vHex.size(); n++){
			pOut[n] = vHex[n];
		}
		...
		for(int i = 0; i < GroupCnt; i++){
			...
			lSerial<< vHex.size()/*strSN.GetLength()*//*m_tCfg.MACSNLen*/; //¹¤³§»Ø¸´ÊÇ»ùÓÚSN³¤¶ÈÀ´µÄ¡£

			lSerial.SerialInBuff((BYTE*)pOut, vHex.size());
...
void CDlgSNCustom::initOnce(){
	...
	CString strIniFile;
	memset(TmpBuf,0,512);
	strIniFile.Format("%s\\sngen\\SCS_SN_THTuple_002.ini",GetCurrentPath());
	if (!PathFileExists(strIniFile)){
		AfxMessageBox("�Ҳ���SCS_SN_THTuple_002.ini�����ļ�������!");
		return ;
	}
	...
	m_tCfg.nMode = GetPrivateProfileInt("Config", "Mode ", 0, strIniFile);

	
	memset(TmpBuf,0,512);
	GetPrivateProfileString("AgentOrder", "Domain", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.strDomain.Format("%s", TmpBuf) ; //url

	memset(TmpBuf,0,512);
	GetPrivateProfileString("AgentOrder", "code", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.strCode.Format("%s", TmpBuf) ; 

	memset(TmpBuf,0,512);
	GetPrivateProfileString("AgentOrder", "SecretId", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.strSecretId.Format("%s", TmpBuf) ;
	m_tCfg.strCode;

	memset(TmpBuf,0,512);
	GetPrivateProfileString("AgentOrder", "SecretKey", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.strSecretKey.Format("%s", TmpBuf) ; 

	memset(TmpBuf,0,512);
	GetPrivateProfileString("AgentOrder", "algorithm", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.strAlgorithm.Format("%s", TmpBuf) ; 

	memset(TmpBuf,0,512);
	GetPrivateProfileString("AgentOrder", "OrderID", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.strOrderID.Format("%s", TmpBuf) ; 
	...
	if (bRet == TRUE){
		if (m_DllHelp.m_bLoadDll == false){
			m_FileLog.PrintLog(LOGLEVEL_ERR, "Tuple_TuoHeng.dll����RegisterID�ӿ�ʧ�� "); ////×¢²áÊ§°Ü
		}
	}
