...
bool Sha1::update(const char* input, DWORD size) {
...
bool Sha1::update(const char* input) {
    return update((const char*)input, strlen(input));
}
...
void Sha1::getDigestString(char* output, bool toUpperCase) {
    ...
    for (int i = 0; i < sizeof(origin_output)/sizeof(origin_output[0]); i++) {
        char high = origin_output[i] / 16, low = origin_output[i] % 16;
        output[index] = high < 10 ? '0' + high : (toUpperCase ?('A'+ high - 10)  : ('a' + high - 10));
        output[index + 1] = low < 10 ? '0' + low : (toUpperCase ? ('A'+ low - 10) : ('a' + low - 10));
        index += 2;
    }
}
