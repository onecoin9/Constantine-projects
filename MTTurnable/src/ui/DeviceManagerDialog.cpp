    ...
    for (const auto& deviceId : deviceIds) {
        ...
        switch(device->getType()) {
            case Domain::IDevice::DeviceType::Handler: 
                typeStr = "Handler";
                // 【新增】为Handler设备连接信号，以便自动更新UI
                if(auto handler = std::dynamic_pointer_cast<Domain::HandlerDevice>(device)) {
                    // 先断开旧连接，防止重复连接
                    disconnect(handler.get(), &Domain::HandlerDevice::linkHealthChanged, this, &DeviceManagerDialog::onRefreshDevices);
                    connect(handler.get(), &Domain::HandlerDevice::linkHealthChanged, this, &DeviceManagerDialog::onRefreshDevices);
                }
                break;
        ...
        switch(device->getStatus()) {
            ...
            {
                statusStr = "已连接";
                // 对于HandlerDevice，附加链路健康状态
                if(auto handler = std::dynamic_pointer_cast<Domain::HandlerDevice>(device)) {
                    if (handler->isLinkHealthy()) {
                        statusStr += " (链路健康)";
                    } else {
                        statusStr += " (链路异常)";
                    }
...
void DeviceManagerDialog::updateButtonStates()
{
    ...
    // 对于BurnDevice，允许即使未连接也打开控制面板
    bool allowControlPanel = isControlSupported && 
                           (isInitialized || device->getType() == Domain::IDevice::DeviceType::Burn);
    m_openControlButton->setEnabled(allowControlPanel);
}
...
void DeviceManagerDialog::onDeviceControlRequested(const QString &deviceId)
{
    ...
    // 对于非BurnDevice类型，检查是否已初始化
    if (device->getType() != Domain::IDevice::DeviceType::Burn) {
        bool isInitialized = m_coreEngine->getDeviceManager()->isDeviceInitialized(deviceId);
        if (!isInitialized) {
            QMessageBox::warning(this, "错误", "设备未初始化，请先连接设备。");
            return;
        }
    }
