...
bool CoreEngine::initialize()
{
    ...
    Services::SiteWorkflowRouter::instance().loadFromFile("config/site_workflow_router.json");
...
void CoreEngine::handleWorkflowCompleted(const QString &instanceId)
{
    ...
    if (workflowId.contains("activate")) {
        // 激活成功，通知Handler
        m_dutManager->setState(dutId, Domain::Dut::State::ActivationPassed);
        // 这里可以调用handler设备，发送67码
        auto handler = std::dynamic_pointer_cast<Domain::HandlerDevice>(m_deviceManager->getDevice("handler_1"));
        if(handler) {

            handler->executeCommand("setDoneSite", {{"siteIdx", context->getData("siteIndex").toInt()}, {"result", "PASS_HEX_DATA"}});
        }
...
bool CoreEngine::initializeServices()
{
    ...
    // 创建Dut管理器
    m_dutManager = std::make_shared<Services::DutManager>();
...
std::shared_ptr<Services::DutManager> CoreEngine::getDutManager() const
{
    return m_dutManager;
}
