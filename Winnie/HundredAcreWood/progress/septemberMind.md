...
- **工作流规范统一**: 统一主工作流和站点工作流的修改规范，建立一致的代码结构和调用约定，提高开发效率和维护性。
 ...
 - **阶段显示差异化**: UI 区分“制作流程”与“加载流程”模式（颜色/标签/只读状态），减少操作误解。
...
- **文档版本**: v1.19
- **更新内容**:
   - 调整文档结构：新增“目录”“每日进度日志”章节，恢复正确目录锚点。
   - 移动 09-25 今日完成段落至正确时间顺序位置（跟随 09-22 日志规范草案之后）。
   - 修复顶端误植的日志规范内容与 09-25 段落混入 TOC 的问题。
   - 去除多余分隔线，统一单一 '---' 分隔。
   - 修正日期年份错误（2024-09-04/05/06 → 2025 年）。
   - 新增 09-25 三项：开始按钮联动阶段；阶段显示差异化；QWarning 清理。
   - 版本号升级 v1.19。
   - 保留原有决策、风险、外部依赖、数据库、重试策略等内容未改动。
- **创建人**: 开发团队
