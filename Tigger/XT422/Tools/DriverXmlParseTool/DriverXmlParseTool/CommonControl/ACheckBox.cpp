...
ACheckBox::ACheckBox(QWidget *parent)
	...
	, m_bView(false)
{
	this->setMinimumSize(QCheckBox::size());//Qt默认
...
int ACheckBox::CalCursorCol(QPoint pt)
{
	return (pt.x() < FRAMESHAPE ? 1 : ((pt.x() > this->width() - FRAMESHAPE) ? 3 : 2));
}

int ACheckBox::CalCursorPos(QPoint pt, int colPos)
{
	return ((pt.y() < FRAMESHAPE ? 10 : ((pt.y() > this->height() - FRAMESHAPE) ? 30 : 20)) + colPos);
}

void ACheckBox::setCursorShape(int CalPos)
{
...
int ACheckBox::GetCursorPos()
{
	return m_iCalCursorPos;
}

void ACheckBox::SetView(bool v)
{
	m_bView = v;
}

void ACheckBox::mousePressEvent(QMouseEvent* event)
{
	m_iCalCursorPos = CalCursorPos(event->pos(), CalCursorCol(event->pos()));
	if (event->buttons() == Qt::LeftButton)
	{
		qDebug() << QString::fromLocal8Bit("点击ACheckBox窗口");

		emit sgnCheckBox(this);
	...
	{
		qDebug() << QString::fromLocal8Bit("右键点击ACheckBox窗口");

		emit sgnRClickCheck(this);
	}
...
void ACheckBox::mouseMoveEvent(QMouseEvent* event)
{
	if (m_bView)
		return;
	//窗体不是最大的话就改变鼠标的形状
...
void ACheckBox::mouseReleaseEvent(QMouseEvent* event)
{
