...
//DDR Logical Location
#define DDR_SharedMemeroyExchangeSize_LL		(0x8000000UL)	//128M
#define DDR_SharedMemeroyExchangePC2MUOffset_LL	(0x7800000UL)	//128-8 = 120M

//DDR Physical Location
#define DDR_SharedMemeroyExchangeSize_PL		(0x38000000UL)	//896M
#define DDR_SharedMemeroyExchangePC2MUOffset_PL	(0x3F800000UL)	//1016M 起始填入位置
...
public:
	CACCmdHandler();
	~CACCmdHandler();
	void InitMainWidget(QWidget* _obj = nullptr);
	...
	int32_t RemoteDoPTCmd(std::string devIP, uint32_t HopNum, uint32_t PortID, uint32_t CmdFlag, uint16_t CmdID, uint32_t SKTNum, uint16_t BPUID, QByteArray& CmdDataBytes);
	...
	int32_t SendDevCallBackResult(std::string devIP, uint32_t HopNum, uint32_t PortID, int32_t ResultCode, QByteArray RespData);
	...
	int32_t SSD2DDR(std::string devIP, uint32_t HopNum, uint32_t PortID, uint64_t SrcSSDAddr, uint64_t DestDDRAddr, uint64_t DataSize);
	...
	/// <param name="DataSize">数据长度</param>
	/// <returns>成功返回0，失败返回负数</returns>
	int32_t DDR2SSD(std::string devIP, uint32_t HopNum, uint32_t PortID, uint64_t SrcDDRAddr, uint64_t DestSSDAddr, uint64_t DataSize);
	...
	int32_t GetCapacity(std::string devIP, QString Type,uint32_t HopNum,uint32_t PortID);

	/// <summary>
	/// 读取数据SSD或DDR的数据并保存到指定的文件.该函数只能单线程去执行写文件
	/// </summary>
	...
	/// <returns>成功返回0，失败返回负数</returns>
	int32_t ReadDataAndSaveToFile(std::string devIP, QString strFilePath, QString Type, uint32_t HopNum, uint32_t PortID, uint64_t SrcAddr, uint64_t Length, uint64_t fileOffset, tJsonParaHash& ResultHash);
	...
	/// <param name="ResultHash">反馈的结果，放在这个Hash里</param>
	/// <returns>成功返回0，失败返回负数</returns>
	int32_t WriteDataFromFile(std::string devIP, QString strFilePath, QString Type, uint32_t HopNum, uint32_t PortID, uint64_t DestAddr, tJsonParaHash& ResultHash);

	/// <summary>
	/// 将数据存放到SSD或DDR中
	/// </summary>
	/// <param name="Type">写入SSD还是DDR中，类型判断</param>
	/// <param name="HopNum">请求发送的节点号，也就是AG06位置</param>
	/// <param name="PortID">对应的端口</param>
	/// <param name="SSDAddr">数据被写入到SSD/DDR的哪个位置-物理位置</param>
	/// <param name="pCmdData">请求存放的数据</param>
	/// <param name="CmdDataSize">数据长度</param>
	/// <returns>成功返回0，失败返回负数</returns>
	int32_t StoreDataToSSDorDDR(std::string devIP, QString Type, uint32_t HopNum, uint32_t PortID, uint64_t SSDAddr, QByteArray& DataBytes, std::string strProgSN, std::string strDataType);
	int32_t ReadDataFromSSDorDDR(std::string devIP, QString Type, uint32_t HopNum, uint32_t PortID, uint64_t SSDAddr, int32_t BytesToRead, QByteArray& DataBytes, std::string strProgSN = "", std::string strDataType = "");
	
	// 高性能版本：去掉CRC校验等耗时操作，尽可能减少拷贝
	int32_t ReadDataFromSSDorDDR2(std::string devIP, QString Type, uint32_t HopNum, uint32_t PortID, uint64_t SSDAddr, int32_t BytesToRead, QByteArray& DataBytes);

	int32_t LinkScan(std::string devIP, uint32_t HopNum);

	int32_t CloseProgramSocket();

	uint8_t GetNextSeqNum();
public slots:
	//透传包执行完成,发出该信号，HopNum完成命令的节点号，PortID，对应的PortID，ACCmdCompletePack为结果包
	void onPTPacketExecComplete(QString, qint32, qint32, QByteArray);
	//AG06请求执行透传包命令,发出该信号，HopNum完成命令的节点号，PortID，对应的PortID，ACCmdQueryPack为请求包
	void onQueryDoPTCmd(QString, qint32, qint32, QByteArray);
signals:
	//当AG06完成RemoteDoCmd命令之后，如果没有通过RegistCbkCmdComplete注册回调函数，则会发送sigCmdComplete信号
	void sigRemoteCmdComplete(QString recvIP, uint32_t HopNum, uint32_t PortID, int32_t ResultCode, uint16_t CmdID, uint32_t SKTEn, uint16_t BPUID, QByteArray RespDataBytes, uint32_t RespDataSize);
	//当AG06有请求PC的命令到达之后，如果没有通过RegistCbkQueryDoCmd注册回调函数，则会发送sigRemoteQueryDoCmd信号
	void sigRemoteQueryDoCmd(QString recvIP, uint32_t HopNum, uint32_t PortID,uint32_t CmdFlag, uint16_t CmdID, uint16_t BPUID, QByteArray CmdDataBytes);
	//临时写文件函数线程关闭
	void sgnThreadExit(int);
...
private:
	...
	std::atomic<uint8_t> currentSeqNum;
	QMutex m_SSDMutex;
	uint64_t test = 0;
};
