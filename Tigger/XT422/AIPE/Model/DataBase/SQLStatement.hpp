...
private:
    ...
    //std::shared_ptr<std::vector<std::string>> lastList;
...
public:
    ...
    void Clear()
    {
		sets.clear(); select.clear(); tables.clear(); join.clear();
		innerJoin.clear(); outerJoin.clear(); leftOuterJoin.clear();
		rightOuterJoin.clear(); where.clear(); having.clear(); groupBy.clear();
		orderBy.clear(); columns.clear(); valuesList.clear();
		if (lastList) {
			lastList->clear();
			delete lastList;
			lastList = nullptr;
