...
BEGIN_MESSAGE_MAP(CID206App, CWinApp)
END_MESSAGE_MAP()
...
BOOL SNCreateWindow(DRVSNCFGPARA *pSnCfgPara,RECT Position)
{
	...
	CDlgID206 *pDlgSN=new CDlgID206(pSnCfgPara);
	...
	if(pSnCfgPara->pParent!=NULL){
		Ret=pDlgSN->Create(CDlgID206::IDD,pSnCfgPara->pParent);
		if(Ret){
...
{
	CDlgID206 *pDlgSN=reinterpret_cast<CDlgID206*>(pSnCfgPara->DrvPrivData);
	if(pDlgSN){
		delete pDlgSN;
