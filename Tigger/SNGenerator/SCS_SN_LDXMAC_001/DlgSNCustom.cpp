...
BOOL CDlgSNCustom::ConnectAndSend(CString strSendData)
{
	...
	if (Ret == TRUE){
		if (send(m_sock,/*(LPCTSTR)*/ (LPSTR)(LPCSTR)strSendData, strSendData.GetLength(), 0) == -1){
			AfxMessageBox("烧录结果发送给服务器失败，请检查下网络环境！");
		}		
	}else{
		AfxMessageBox("连接服务器失败，烧录结果将无法上传至服务器，请检查网络环境！");
	}
...
INT CDlgSNCustom::QuerySN(UINT AdapIdx,DWORD Idx,BYTE*pData,INT*pSize)
{
	...
	item.index = Idx;
	item.strStartTime.Format("%s", strTime);
	item.strMac = "";
	
	//strURL.Format("http://%s",m_tCfg.DownloadIPAddr);
	//strTemp.Format(":%u/Interface/GetTRACKNUMBERMAC?gdh=%s", m_tCfg.DownloadIPPort, 
	//	m_tCfg.GDHPara);
	//strURL += strTemp;
	strURL.Format("%s", m_tCfg.URLPara);
	

	Ret = Client.HttpGet(strURL, "", "", strResponse);
	m_FileLog.PrintLog(LOGLEVEL_ERR, "QuerySN strURL=%s, strResponse=%s, HttpGet Ret=%d",strURL, strResponse, Ret );
	if (Ret != 0){
		if (Ret==1){
		}else if (Ret==2){
		}
	...
	//strResponse.Format("1CD6BDFF0029");
	strResponse.Remove('"');
	strSN.Format("%s", strResponse);
	item.strMac.Format("%s", strSN);

	item.nTime = time(NULL);

	/*vHex.clear();
	ComTool::Str2Hex(strSN, ComTool::ENDIAN_BIG, vHex);*/
	...
	/*ComTool::Hex2StrNew(ComTool::ENDIAN_BIG,pData, Ret, strData);
	PrintLog(LOGLEVEL_WARNING,"SNQuery pData = %s", strData);*/

__end:

	m_vReport.push_back(item);
...
void CDlgSNCustom::initOnce(){
	...
	CString strIniFile;
	memset(TmpBuf,0,512);
	strIniFile.Format("%s\\sngen\\SCS_SN_LDXMAC_001.ini",GetCurrentPath());
	if (!PathFileExists(strIniFile)){
		AfxMessageBox("找不到SCS_SN_LDXMAC_001.ini配置文件，请检查!");
		return ;
	}

	GetPrivateProfileString("Config", "URLPara", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.URLPara.Format("%s", TmpBuf);//all url

	GetPrivateProfileString("Config", "DownloadIPAddr", "",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.DownloadIPAddr.Format("%s", TmpBuf);

	m_tCfg.DownloadIPPort = GetPrivateProfileInt("Config", "DownloadIPPort ",1028, strIniFile);

	GetPrivateProfileString("Config", "GDHPara","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.GDHPara.Format("%s", TmpBuf);

	GetPrivateProfileString("Config", "UploadIPAddr","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.UploadIPAddr.Format("%s", TmpBuf);

	m_tCfg.UploadIPPort = GetPrivateProfileInt("Config", "UploadIPPort ",1018, strIniFile);
	
	CString strTemp;
	GetPrivateProfileString("Config", "MACSNAddr", "",TmpBuf, MAX_PATH, strIniFile);
	strTemp.Format("%s", TmpBuf);
	sscanf(strTemp, "%I64X", &m_tCfg.MACSNAddr);
	
	m_tCfg.MACSNLen = GetPrivateProfileInt("Config", "MACSNLen ",16, strIniFile);
	
	GetPrivateProfileString("Config", "WorkOrderID","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.WorkOrderID.Format("%s", TmpBuf);

	GetPrivateProfileString("Config", "CachePath","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.CachePath.Format("%s", TmpBuf);

	GetPrivateProfileString("Config", "_MATNR","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._MATNR.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_TYPE1","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._TYPE1.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_DOTYPE","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._DOTYPE.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_USERID","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._USERID.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_FILEVERSION","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._FILEVERSION.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_RemoteUpdate","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._RemoteUpdate.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "Datecode","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.Datecode.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "KehuName","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.KehuName.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "ModelID","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.ModelID.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "Version","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.Version.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "FwId","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.FwId.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "DeviceInfo","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.DeviceInfo.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "Token","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.Token.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "KeyType","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.KeyType.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "TestLightcount","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.TestLightcount.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_MultiLightIndex","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._MultiLightIndex.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "_TestLightIndex","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg._TestLightIndex.Format("%s", TmpBuf);
	GetPrivateProfileString("Config", "PC_MAC","",TmpBuf, MAX_PATH, strIniFile);
	m_tCfg.PC_MAC.Format("%s", TmpBuf);


	m_FileLog.PrintLog(LOGLEVEL_ERR, "URLPara=%s, DownloadIPAddr =%s, DownloadIPPort=%u, GDHPara=%s, UploadIPAddr=%s,\
									 UploadIPPort=%u, MACSNAddr=0x%I64X,  MACSNLen=%u, WorkOrderID=%s, CachePath=%s", m_tCfg.URLPara,
									 m_tCfg.DownloadIPAddr, m_tCfg.DownloadIPPort, m_tCfg.GDHPara,  m_tCfg.UploadIPAddr,
									 m_tCfg.UploadIPPort, m_tCfg.MACSNAddr, m_tCfg.MACSNLen, m_tCfg.WorkOrderID, m_tCfg.CachePath);
