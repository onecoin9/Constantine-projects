...
// 获取单例实例的静态方法
MTUIDGenerator& MTUIDGenerator::getInstance() {
    // 使用双检锁模式确保线程安全的单例初始化
    if (instance_ == nullptr) {
        std::lock_guard<std::mutex> lock(instanceMutex_);
        if (instance_ == nullptr) {
            std::shared_ptr<ITimeProvider> timeProvider = std::make_shared<RealTimeProvider>();
            instance_ = new MTUIDGenerator(timeProvider);
        }
    }
    return *instance_;
}
...
// 在程序退出时释放单例资源
// 注意：这需要在适当的地方调用，如程序退出前
void cleanupMTUIDGenerator() {
    std::lock_guard<std::mutex> lock(MTUIDGenerator::instanceMutex_);
    if (MTUIDGenerator::instance_ != nullptr) {
        delete MTUIDGenerator::instance_;
        MTUIDGenerator::instance_ = nullptr;
    }
}
