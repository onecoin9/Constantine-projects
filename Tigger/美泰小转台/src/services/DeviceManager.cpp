    ...
    if (m_dutManager) {
        ...
        if (testBoard) {
            connect(testBoard.get(), &Domain::TestBoardDevice::testDataAvailable,
                    this, [this](const QString& dutId, const QVariantMap& data){
                        if (m_dutManager) {
                            m_dutManager->setTestData(dutId, data);
                        }
                    });
        }
    }
...
std::shared_ptr<Domain::IDevice> DeviceManager::getBurnDevice() const
{
    for (auto it = m_devices.begin(); it != m_devices.end(); ++it) {
        if (it.value() && it.value()->getType() == Domain::IDevice::DeviceType::Burn) {
            return it.value();
        }
    }
...
std::shared_ptr<Domain::IDevice> DeviceManager::getHandlerDevice() const
{
    for (auto it = m_devices.begin(); it != m_devices.end(); ++it) {
        if (it.value() && it.value()->getType() == Domain::IDevice::DeviceType::Handler) {
            return it.value();
        }
    }
...
void DeviceManager::setDutManager(DutManager* dutManager)
{
...
std::shared_ptr<Domain::IDevice> DeviceManager::createBurnDevice(const QJsonObject &config)
{
    ...
    if (m_dutManager) {
       connect(device.get(), &Domain::BurnDevice::deviceDiscovered,
               m_dutManager, &DutManager::updateSiteFromScan);
       connect(device.get(), &Domain::BurnDevice::getSKTEnable,
               m_dutManager, &DutManager::updateSktFromDownLoadProject);
       // DoJob结果: 更新站点统计 + 指定 DUT 的 lastJobResult
       connect(device.get(), &Domain::BurnDevice::doJobResult,
               this, [this](const QString& ip, int nHopNum, const QJsonObject& detail){
            Q_UNUSED(detail)
            auto siteMap = m_dutManager->getAllSiteInfo();
            for (auto it = siteMap.begin(); it != siteMap.end(); ++it) {
                if (it.value().ip == ip) {

                    QJsonArray resultArray = detail["dutResult"].toArray();
                    for (int i = 0; i < resultArray.size(); i++) {
                        bool success = resultArray[i].toInt() == 1;
                        int siteIndex = i;
                        m_dutManager->updateSiteTestResult(it.value().siteIndex, success);
                        int socketOneBased = i + 1;
                        if (socketOneBased > 0) {
                            QString dutId = QString("%1_Dut_%2").arg(it.key()).arg(socketOneBased);
                            QVariantMap data;
                            data["lastJobResult"] = success ? "Passed" : "Failed";
                            data["ip"] = ip;
                            data["socketIndex"] = socketOneBased;
                            m_dutManager->setTestData(dutId, data);
                        }
                        break;
                    }
