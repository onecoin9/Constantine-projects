...
bool SProtocol::sendPduCommand(uint16_t pflag, uint8_t pdu, const QByteArray& data)
{
    ...
    QByteArray responseData;
    bool success = m_clientChannel->sendToPortAsync(packet, m_commandPort, m_vautoHost, m_connectTimeout, &responseData);
    if (!success) {
...
bool SProtocol::sendAxisMoveCommand(const QJsonObject& command)
{
    ...
    QByteArray responseData;
    bool success = m_clientChannel->sendToPortAsync(jsonData, m_commandPort, m_vautoHost, m_connectTimeout, &responseData);
    if (!success) {
...
bool SProtocol::sendJsonCommand(const QJsonObject& command)
{
    ...
    return m_clientChannel->sendToPortAsync(jsonData, m_commandPort, m_vautoHost, m_connectTimeout);
}
...
QByteArray SProtocol::buildPduPacket(uint16_t pflag, uint8_t pdu, const QByteArray& data)
{
    ...
    // PLEN(1/2字节) - 数据长度
    if (pdu == PDU_TELLPRINT) {
        packet.append(static_cast<short>(data.size()));
    }
    else {
...
{      
    QtConcurrent::run([this, data]() {
        processReceivedData(data);  // 在线程池中执行
    });
}
