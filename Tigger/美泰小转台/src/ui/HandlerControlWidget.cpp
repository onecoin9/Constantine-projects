...
void HandlerControlWidget::createTaskControlGroup()
{
    ...
    // 连接信号 - 使用异步方式调用文件对话框，避免在主线程中可能触发的死锁问题
        ...
        QTimer::singleShot(0, this, [this]() {
            try {
                ...
                if (!filePath.isEmpty()) {
                    ...
                    LOG_INFO(QString("选择的任务文件路径: %1").arg(filePath).toStdString());
                } else {
                    LOG_INFO("用户取消了任务文件选择");
                }
            } catch (const std::exception& e) {
                LOG_ERROR(QString("选择任务文件时发生异常: %1").arg(e.what()).toStdString());
                QMessageBox::critical(this, "错误", QString("选择文件时发生错误: %1").arg(e.what()));
            } catch (...) {
                LOG_ERROR("选择任务文件时发生未知异常");
